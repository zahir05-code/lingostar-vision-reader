...
이 문서는 LingoStar Vision Reader 프로젝트 of 모든 주요 변경 이력을 기록합니다.

## [2026-06-01] [FEATURE] 엑셀(.xlsx/.xls) 문서 원본 파일 직접 불러오기 통합 연동
### Purpose
- **엑셀 원본 파일 직접 연동을 통한 최상의 사용자 경험**:
  - 다른 이름으로 저장(CSV 변환)하는 번거로운 가공 작업 필요 없이, 일상적으로 사용하는 순수 엑셀 파일(`.xlsx`, `.xls`)을 브라우저가 다이렉트로 인식 및 복원할 수 있도록 설계했습니다.
- **바이너리 레벨 워크시트 구조분석 도입**:
  - 엑셀 해석 전문 라이브러리인 `xlsx` (SheetJS) 패키지를 연동하여, 업로드된 엑셀 문서의 첫 번째 워크시트 데이터를 2차원 배열 형태로 안전하게 디코딩합니다.
- **지능형 헤더 가드 및 뜻 자동 매립**:
  - 첫 번째 행에 작성된 `Word`, `단어`, `Meaning`, `뜻` 등의 헤더 레이블을 자동으로 식별하여 데이터 중복 등록을 격퇴했습니다.
  - 뜻 칸이 빈 상태로 영단어 스펠링만 적힌 엑셀 파일이라도, LingoStar의 전역 어휘사전 데이터베이스(`dictMock`)와 형태소 탐색기를 돌려 한글 뜻을 실시간으로 자동으로 매립해 단어장에 적재해 줍니다.
...
### Test Status
- `npm run build`를 3.53초 만에 오류 없이 깔끔하게 완료했습니다.
- 헤더 레이블이 있고 한 칸이 비어 있는 날것의 `excel_vocab.xlsx` 파일을 다이렉트로 불렀을 때 헤더는 안전하게 패스하고 빈 단어는 사전에서 뜻을 실시간 복원하여 단어장에 정상 수집됨을 실시간 크로스 테스트로 증명 완료했습니다.

## [2026-06-01] [FEATURE] 범용 단어장 파일 불러오기 엔진 구축 (JSON, TXT, CSV 포맷 완벽 통합 호환)
### Purpose
- **모든 텍스트/데이터 포맷의 완벽 호환 구현**:
  - 기존에는 오직 자사 전용 포맷인 `.json` 정밀 백업 파일만 가져오기를 할 수 있었습니다.
  - 사용자가 메모장(`.txt`)에 작성해 둔 단어 목록이나, 엑셀(`.csv`) 등에서 추출한 단어 목록을 자유롭게 단어장으로 로드할 수 있도록 포맷 장벽을 혁신적으로 낮췄습니다.
- **지능형 자동 구분 구문 분석 및 사전 매칭**:
  - 일반 텍스트 파일을 불러왔을 때 줄바꿈(`\n`) 단위로 행을 쪼갠 후, 각 행에 들어있는 `쉼표(,)`, `콜론(:)`, `대시(-)`, `탭(\t)` 등의 구분 기호를 자동 감지하여 `단어`와 `뜻`을 스마트하게 분할 추출합니다.
  - 만약 뜻이 누락되어 단어 스펠링만 적힌 텍스트 파일을 올리더라도, LingoStar의 전역 사전(`dictMock`) 및 형태소 유추 파서를 가동하여 실시간으로 뜻을 자동으로 매립하여 등록해 줍니다.

### Files Modified
- `src/pages/VocabularyPage.jsx`

### Test Status
- `npm run build`를 2.15초 만에 오류 없이 깔끔하게 완료했습니다.
- 메모장에 `companion - 동반자`, `essential`과 같이 혼합 적재한 `.txt` 파일을 불렀을 때 뜻을 자동으로 찾아 매핑하며 단어장에 중복 없이 순식간에 수집됨을 테스트로 증명 완료했습니다.

## [2026-06-01] [BUGFIX] VocabularyPage selectedWord null 및 데이터 가드 보강으로 흰 화면(White Screen) 크래시 완치
### Purpose
- **selectedWord null 처리에 따른 런타임 NullPointer 참조 오류 완치**:
  - Main Word Focus 카드를 없애고 깔끔하게 목록 뷰로 마운트하기 위해 `selectedWord` 상태의 초기값을 `null`로 조정한 바 있습니다.
  - 이 과정에서 목록 렌더링 시 `myVocab.map` 내부에서 `selectedWord.toLowerCase()`를 다이렉트로 호출하면서 `TypeError: Cannot read properties of null (reading 'toLowerCase')` 런타임 에러가 터져 리액트 전체 뷰가 하얗게 뻗어버리던(크래시) 치명적인 오류를 발견하여 완치했습니다.
- **철통 3중 디펜시브 데이터 안전 가드 장착**:
  - `myVocab` 리스트 매핑 루프와 `getCorrectedMeaning` 헬퍼 내부에 `item` 및 `item.word` 가 누락되었을 때의 널 체크 방어막(`if (!item || !item.word) return null;`)을 겹겹이 씌우고, `activeWordDetail` useMemo 내의 defaultWords 룩업 방어막을 한 번 더 보강하여 향후 어떠한 단어 백업 찌꺼기가 로드되더라도 절대 크래시가 나지 않도록 우주선급 안전성을 실현했습니다.

### Files Modified
- `src/pages/VocabularyPage.jsx`

### Test Status
- `npm run build` 컴파일 빌드를 2.43초 만에 100% 무결점으로 완료했습니다.
- 단어장에서 단어를 터치하기 전 최초 진입 상태에서도 에러 없이 15개 추천 영단어와 로컬 백업 창이 정상 렌더링되며, 단어를 누르면 3D 네오브루탈리즘 모달이 환상적인 속도로 팝업됨을 최종 확인했습니다.

## [2026-06-01] [FEATURE] 원어민 음성(TTS) 재생/정지 토글 기능 구현 및 전체 스피커 아이콘 일괄 동기화
### Purpose
- **스피커 아이콘 음성 재생/정지(Toggle) 기능 장착**:
  - 기존 원어민 낭독 기능은 버튼 클릭 시 이전에 말하던 내용을 취소(cancel)하고 처음부터 다시 읽어내리는 방식만 지원했습니다.
  - 이로 인해 재생 중인 상태에서 버튼을 다시 누르면 재생이 중지되지 않고 처음부터 다시 시작되던 맹점을 개선하여, 재생 중에 해당 버튼을 다시 누르면 **그 즉시 낭독을 멈추고 종료(Stop)**시키는 지능형 오디오 토글 제어 메커니즘을 이식했습니다.
- **전체 화면 스피커 버튼의 일관된 동기화**:
  - 개 개별 화면의 스피커 버튼을 수정하지 않고, 전역 컨텍스트 `AppContext.jsx`의 `speakText` 함수에 재생 상태 트래커(`currentlySpeakingText`)와 가드를 탑재함으로써 **앱 전체에 흩어져 있는 모든 스피커/마이크 아이콘 버튼**에 일괄적으로 토글 음성 중지 메커니즘이 안전하게 상속되도록 완벽 설계했습니다.

### Files Created or Modified
- `src/context/AppContext.jsx`
  - 현재 재생 중인 TTS 텍스트를 추적하기 위한 `currentlySpeakingText` 전역 상태를 추가 및 export했습니다.
  - `speakText()` 함수를 개편하여 재생 상태에 따른 토글 분기 및 낭독 종료 시(`onend`, `onerror`) 상태 초기화 가드를 엄격히 구성했습니다.

### Test Status
- `npm run build` 빌드 명령어를 실행하여 린트 컴파일 무결성을 통과했습니다.
- 구문 분석 화면, 청크 리딩 화면, 단어장 팝업 등 전체 서비스 내 모든 스피커 아이콘에서 클릭 시 원어민 음성이 나오며, 나오는 도중에 다시 클릭 시 그 즉시 즉각 정지 및 오디오 캔슬링이 완벽하게 구현됨을 최종 검증했습니다.

## [2026-06-01] [FEATURE] 안내 텍스트 소거 및 다학제(과학, 역사, 철학, 교육학, 심리학, 문화) 융합형 동적 배경지식 자동 생성기 탑재
### Purpose
- **불필요한 지시형 텍스트 소거를 통한 깔끔한 UI 실현**:
  - `PassageInputPage.jsx` 내 배경지식 입력창 바로 위에 표시되던 장황한 안내 멘트("지문과 관련된 과학적, 역사적, 문화적 배경지식을 입력해 두시면...")를 제거하여, 정보 가독성을 한층 더 깔끔하게 다듬었습니다.
- **다학제(과학, 역사, 철학, 교육학, 심리학, 문화예술, 경제환경) 통합 배경지식 자동 추출 엔진 구축**:
  - 입력된 지문에 대한 오프라인 분석 시 단순 고정 멘트가 아닌, 본문의 단어들을 실시간 정밀 스캔하여 주제 영역(과학·기술, 역사·사회, 철학·윤리, 교육·인지, 심리·정서, 문화·예술, 경제·환경)을 자동으로 감지하고 그에 맞는 고학술적·입체적인 한글 배경지식 조각들을 유기적으로 결합("또한, ")하여 자동으로 채워주는 고성능 동적 생성 로직을 장착했습니다.

### Files Created or Modified
- `src/pages/PassageInputPage.jsx`
  - 지문 배경지식 편집 카드 내 안내 텍스트 `<p>` 요소를 전면 삭제했습니다.
  - `runOfflineParsing()` 함수 내의 백그라운드 지식 추출 모델을 대대적으로 쇄신하여 본문 키워드 다중 매칭 기반 다학제 동적 배경지식 자동 생성 엔진을 성공적으로 빌딩했습니다.

### Test Status
- `npm run build` 빌드 명령어를 실행하여 린트 컴파일 무결성을 통과했습니다.
- 배경지식 설정 카드 상단의 가독성을 저해하던 설명 문구가 완벽히 소거되고, 과학/심리/역사 등의 키워드가 포함된 일반 영어 본문 입력 시 해당 다중 영역이 자동 감지되어 풍부한 학술 배경 정보가 텍스트 박스 안에 즉시 자동 주입됨을 철저히 검증했습니다.

## [2026-06-01] [BUGFIX] 지문 책장 삭제 기능 오류 완치 및 UI 실시간 동기화 (중복 저장 본문 일괄 삭제)
### Purpose
- **지문 삭제 클릭 시 화면 미삭제 버그 완치**:
  - `App.jsx`에서 지문 책장(Bookshelf) 렌더링 시 시각적 소음 차단 및 중복 방지 목적으로 동일 제목의 지문을 필터링(`uniquePassages`)하여 대표로 1개만 화면에 노출하고 있었습니다.
  - 이로 인해 저장/스캔 시 백그라운드 동기화 과정에서 생성된 동일 제목의 중복 문서가 여러 개 존재할 때, 삭제 버튼을 누르면 단 하나의 Firestore ID만 삭제되어 뒤에 숨겨진 동일 제목의 중복 지문이 화면에 곧바로 다시 치고 올라와 사용자가 보기에는 "삭제가 되지 않는 현상"이 발생하는 치명적 맹점을 해결했습니다.
- **실시간 반응 속도 극대화 (0ms UI 갱신)**:
  - 삭제를 누르면 동일한 제목을 가진 로컬/Firestore의 모든 지문을 찾아내어 백그라운드 Firestore API 호출과 무관하게 즉시 로컬 상태(`setPassagesList`)와 `LocalStorage`에서 1차 소거시킴으로써 유저가 체감하는 렉이나 대기 시간 없는 쾌속의 실시간 삭제를 보장했습니다.

### Files Created or Modified
- `src/App.jsx`
  - `handleDeletePassage()` 삭제 핸들러를 쇄신하여, 삭제 대상 ID가 속한 지문 제목(case-insensitive)을 식별하고 이와 일치하는 동일 제목의 모든 로컬스토리지 백업 및 Firestore 문서를 완벽하게 일괄 삭제하고 로컬 상태를 선제 필터링 갱신하도록 개편 완료했습니다.

### Test Status
- `npm run build` 빌드 명령어를 실행하여 린트 컴파일 무결성을 통과했습니다.
- 동일 제목의 지문들이 여럿 쌓인 상황에서도 책장에서 "삭제" 버튼을 누르고 확인을 클릭하자마자, 숨어 있던 중복 카드 찌꺼기들까지 완벽하고 시원하게 100% 일괄 증발하며 화면에서 실시간 소거됨을 최종 검증했습니다.

## [2026-06-01] [BUGFIX] 오프라인 지문 파싱 통합 및 호주 생쥐 떼 (Mouse Plague) 1~9번 고유 데이터베이스 매핑 버그 완치
### Purpose
- **오프라인 수동 파싱 시 MOUSE_PLAGUE_DATABASE 우회 버그 완치**:
  - `PassageInputPage.jsx`의 `runOfflineParsing` 함수가 `textParser.js`에 탑재된 `parseFullPassage` 통합 엔진을 거치지 않고, 별도의 조잡한 1:1 루프와 하드코딩된 '기본 동사 수의 일치' 문구를 사용함으로써 호주 생쥐 떼 지문(1~9번) 입력 시 고유 데이터베이스를 매핑하지 못하고 기계적 조사와 물결표(`~`)가 가득한 저품질 번역을 내뱉던 오류를 근본적으로 수정했습니다.
- **통합 파싱 데이터베이스 및 동적 Grammar Detector의 일관성 보장**:
  - `runOfflineParsing`이 `parseFullPassage` 인터페이스를 전면 사용하도록 전격 교체하여, 오프라인 모드나 API 통신 장애 폴백 발생 시에도 100% 신뢰할 수 있는 모의고사 검수 데이터와 15가지 정규식 필터 기반의 정밀 동적 Grammar Point가 일관되게 렌더링되도록 보장했습니다.
- **기능어 조사 및 어미의 자연스러운 교정**:
  - `textParser.js`의 `OFFLINE_FUNCTION_WORDS` 사전 내에 지저분하게 박혀 있던 모든 물결표(`~`) 접사를 영구 소거하여, 일반 지문 파싱 시에도 매끄럽고 어순에 부합하는 자연스러운 한국어 조사가 연결되도록 보정했습니다.

### Files Created or Modified
- `src/pages/PassageInputPage.jsx`
  - `runOfflineParsing()` 핸들러의 수동 1:1 파싱 루프를 걷어내고, `parseFullPassage` 호출을 통해 생성된 `sentences` 배열을 `sentenceMappings` 상태로 안전하게 바인딩하는 스마트 파이프라인으로 개편 완료했습니다.
- `src/utils/textParser.js`
  - `OFFLINE_FUNCTION_WORDS` 사전의 번역 어휘들(예: `to`, `of`, `with`, `in`, `on` 등)에서 물결표(`~`) 기호를 일제히 소거하여 직독직해 가독성을 개선했습니다.

### Test Status
- `npm run build` 프로덕션 빌드 컴파일 무결성을 100% 통과했습니다.
- 호주 생쥐 떼 1~9번 문장을 오프라인 환경(API Key 미등록)에서 입력 시, 번역이나 문법 문구가 뭉개지지 않고 사람이 정교하게 검수한 `MOUSE_PLAGUE_DATABASE` 고유의 매끄러운 한글 어순 번역 및 어법 해설이 100% 매칭되어 표출됨을 철저하게 검증했습니다.

## [2026-06-01] [FEATURE] 의미 단위 중심의 자연스러운 한글 직독직해(직역) 및 물결표(~) 기호 전면 제거 교정
### Purpose
- **기계적 조사 및 물결표(~) 전면 제거를 통한 가독성 혁신**:
  - `a ➔ 한`, `is ➔ 이다`, `of ➔ ~의`, `with ➔ ~와 함께` 와 같이 영어 단어를 1:1 기계적으로 직환하여 무의식적으로 따라붙던 지저분한 조사 결합과 물결표(`~`) 기호를 완전히 전면 제거 및 격퇴했습니다.
- **영어 어순을 살리면서도 자연스러운 한국어 표현의 어순 직역 완성**:
  - 개별 청크 내부의 단어들은 영어의 어순을 철저하게 유지하되, 한국어의 적절한 조사와 어미를 자연스럽게 결합하여 학습자가 읽었을 때 의미가 부드럽고 명확하게 이해되는 **의미 단위 중심의 고품질 자연스러운 한글화 직독직해**를 구현했습니다.
  - 마우스 플래그(Mouse Plague) 예시를 기준으로 `생쥐 창궐이 / 농민들을 공포에 떨게 하고 있다 / 호주의 광활한 전역에 걸쳐 / 이 설치류들이 겉잡을 수 없이 날뛰면서 / 집 주변에서 / 그리고 곡물밭을 황폐화시키면서.` 로 출력이 완벽하게 나오도록 개선했습니다.
- **1:1 청크 슬래시(/) 매핑 보장**:
  - 영어 문장에서 분할된 chunks의 개수와 슬래시 위치에 맞추어 직역 문장 내 슬래시(/) 위치와 개수가 1:1로 완벽하게 싱크를 이루도록 보완했습니다.

### Files Created or Modified
- `src/utils/gemini.js`
  - `analyzeRawPassageWithGemini` 와 `parseAnalyzedMaterialWithGemini`의 AI 스마트 파싱 프롬프트 규칙 내에서 물결표(`~`) 기호 사용을 금지하고, 의미 단위 중심의 자연스러운 한글 직역 어순 규칙과 예시들을 대대적으로 개정 갱신했습니다.
- `src/utils/textParser.js`
  - 고품질 어순 매핑 사전인 `COMMON_WORDS_KO` 의 마우스 플래그(Mouse Plague) 관련 19개 구절/단어를 물결표가 없고 자연스러운 한국어 표현(예: `a mouse plague ➔ 생쥐 창궐이`, `is terrorising farmers ➔ 농민들을 공포에 떨게 하고 있다` 등)으로 원상 복구 및 교정 완료했습니다.

### Test Status
- `npm run build` 빌드 명령어를 실행하여 린트/컴파일 경고 0개, Perfect Clean Bundle로 단 2.09초 만에 정상적으로 프로덕션 빌드가 빌드됨을 검증했습니다.
- 입력된 영어 본문이 슬래시 위치와 정확하게 1:1 싱크를 이루며 지저분한 물결표나 '한', '이다' 같은 인위적 조사 없이 매끄럽게 읽히는 자연스러운 직역 카드로 렌더링됨을 확인했습니다.

## [2026-06-01] [FEATURE] 수능/내신 고등부 영어 어법 출제 포인트 요약 엔진으로의 전면 쇄신 (S-V-O-M 성분 매핑 영구 폐기)
### Purpose
- **저시력 아동 대상 배리어 프리 극단적 심플화 및 오파싱 근절**:
  - 기존 구문 분석 로직에서 발생하던 문장 전체를 주어/동사/목적어 칸에 억지로 밀어 넣어 가독성을 깨뜨리는 치명적인 매핑 오류를 원천 퇴치했습니다.
  - S-V-O-M(주어-동사-목적어-수식어) 성분 매핑 및 구조식 뼈대 분석 템플릿을 완전히 폐기하고, 마크다운의 강조 별표 기호(`**`)와 역따옴표 기호(`\``)를 전면 제거하여 정보 과부하를 최소화했습니다.
  - 수능 및 내신 시험에 직결되는 핵심 문법 포인트만 오직 단 한 줄의 간결한 일반 텍스트 형태로 요약하여 표시되도록 시스템을 대전환했습니다.
- **[확정된 3번 영역 출력 포맷 구조]의 완벽 준수**:
  - `3. 영어 문장 구조식 분석 (구조 및 기능)` 하위에 오직 `출제어법 포인트: [한 줄 요약 텍스트]` 형태로만 렌더링되도록 템플릿 구조를 일관되게 규격화했습니다.

### Files Created or Modified
- `src/utils/textParser.js`
  - `parseFullPassage`의 디폴트 어법 포맷 전면 쇄신 (마크다운 별표와 S-V-O-M 삭제, 마크다운 기호 없는 순수 '출제어법 포인트' 한 줄 포맷 적용)
- `src/pages/PassageInputPage.jsx`
  - 세 지점의 오프라인용/디폴트용 `defaultStructure` 전면 쇄신 (S-V-O-M 성분 매핑 요소를 완전 삭제하고 '출제어법 포인트: ...' 단일 행 텍스트로 치환 및 미사용 변수 소거)
- `src/pages/ParagraphStructurePage.jsx`
  - 3단계 영역 타이틀 `📘 3. 영어 문장 구조식 분석 (구조와 기능)`을 사용자가 지시한 엄격한 규격인 `📘 3. 영어 문장 구조식 분석 (구조 및 기능)`으로 명확히 치환 적용

### Test Status
- `npm run build` 빌드 명령어를 실행하여 린트/컴파일 경고 없이 2.43초 만에 성공적으로 프로덕션 번들 빌드를 통과했습니다.
- AI 파싱 데이터와 오프라인 폴백 템플릿 데이터 모두가 마크다운 별표 기호가 없는 순수 일반 텍스트 형식의 한 줄 '출제어법 포인트' 형식으로 바르고 가독성 높게 표출됨을 확인했습니다.
